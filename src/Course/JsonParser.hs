...
jsonString ::
  Parser Chars
jsonString = between (is '"') (charTok '"') $ list $ do
  c <- character
  case c of '"' -> unexpectedCharParser '"'
            '\\' -> hexu ||| jsonSpecial
            _ -> valueParser c

jsonSpecial ::
  Parser Char
jsonSpecial = P $ \(c :. cs) -> case fromSpecialCharacter <$> toSpecialCharacter c of Empty -> UnexpectedChar c
                                                                                      Full c_ -> Result cs c_
...
jsonNumber ::
  Parser Rational
jsonNumber = P $ \s -> case readFloats s of Empty -> UnexpectedString s
                                            Full (n, s_) -> Result s_ n
