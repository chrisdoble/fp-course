...
convertInteractive ::
  IO ()
convertInteractive = putStrLn "Enter a string:" >-
  getLine >>= \l ->
  putStrLn $ toUpper <$> l
...
reverseInteractive ::
  IO ()
reverseInteractive = putStrLn "Enter a source file:" >-
  getLine >>= \source ->
  putStrLn "Enter a target file:" >-
  getLine >>= \target ->
  readFile source >>= \content ->
  writeFile target $ reverse content
