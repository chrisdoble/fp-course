...
toList ::
  ListZipper a
  -> List a
toList (ListZipper Nil m r) = m :. r
toList (ListZipper (l :. ls) m r) = toList $ ListZipper ls l (m :. r)
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper l m r) = case break f l of
  (_, Nil) -> IsNotZ
  (r', m' :. l') -> IsZ $ ListZipper l' m' (reverse r' ++ m :. r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper l m r) = case break f r of
  (_, Nil) -> IsNotZ
  (l', m' :. r') -> IsZ $ ListZipper (reverse l' ++ m :. l) m' r'
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z@(ListZipper _ m r) = case moveLeft z of
  IsNotZ -> ListZipper l' m' Nil where (m' :. l') = reverse (m :. r)
  IsZ z -> z
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop z@(ListZipper l m _) = case moveRight z of
  IsNotZ -> ListZipper Nil m' r' where (m' :. r') = reverse (m :. l)
  IsZ z -> z
...
moveLeft (ListZipper (m' :. l') m r) = IsZ $ ListZipper l' m' (m :. r)
...
moveRight (ListZipper l m (m' :. r')) = IsZ $ ListZipper (m :. l) m' r'
...
swapLeft (ListZipper (m' :. l) m r) = IsZ $ ListZipper (m :. l) m' r
...
swapRight (ListZipper l m (m' :. r)) = IsZ $ ListZipper l m' (m :. r)
...
moveLeftN' n z@(ListZipper l _ r) = case moveLeftN n z of
  IsNotZ -> Left $ length $ if n < 0 then r else l
  IsZ z -> Right z
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z@(ListZipper l m r)
  | n < 0 = IsNotZ
  | n >= (length l) + 1 + (length r) = IsNotZ
  | otherwise = IsZ $ ListZipper l' m' r' where
    xs = (reverse l) ++ (m :. r)
    l' = reverse $ take n xs
    m' = headOr undefined $ drop n xs
    r' = drop (n + 1) xs
...
end (ListZipper l m (m' :. r)) = end $ ListZipper (m :. l) m' r
...
start (ListZipper (m' :. l) m r) = start $ ListZipper l m' (m :. r)
...
deletePullLeft (ListZipper (m' :. l') _ r) = IsZ $ ListZipper l' m' r
...
deletePullRight (ListZipper l _ (m' :. r')) = IsZ $ ListZipper l m' r'
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft m' (ListZipper l m r) = ListZipper (m :. l) m' r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight m' (ListZipper l m r) = ListZipper l m' (m :. r)
...
-- /Tip:/ Use `zipWith`
  (ListZipper fs g hs) <*> (ListZipper xs y zs) =
    ListZipper (zipWith ($) fs xs) (g y) (zipWith ($) hs zs)
...
instance Extend ListZipper where
  f <<= z = ListZipper (f <$> l) (f z) (f <$> r) where
    l = unfoldr (\z -> (\x -> (x, x)) <$> toOptional (moveLeft z)) z
    r = unfoldr (\z -> (\x -> (x, x)) <$> toOptional (moveRight z)) z
...
instance Traversable ListZipper where
  traverse f (ListZipper l m r) = ListZipper <$> (traverse f l) <*> (f m) <*> (traverse f r)
...
instance Traversable MaybeListZipper where
  -- Applicative f => (a -> f b) -> MaybeListZipper a -> f (MaybeListZipper b)
