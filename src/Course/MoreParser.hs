...
noneof ::
  Chars
  -> Parser Char
noneof cs = satisfy $ flip notElem cs
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok x y = between (charTok x) (charTok y)
...
hex ::
  Parser Char
hex = do
  ds <- thisMany 4 $ satisfy isHexDigit
  case readHex ds of Empty  -> failed $ UnexpectedString ds
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby p q = sepby1 p q ||| valueParser Nil
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma x y z = betweenCharTok x y $ sepby z (charTok ',')
