...
character ::
  Parser Char
character = P $ \i -> case i of Nil     -> UnexpectedEof
                                a :. i' -> Result i' a
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P p) = P $ \i -> onResult (p i) (\i' a -> Result i' (f a))
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P p) = P $ \i -> onResult (p i) (\i' a -> parse (f a) i')
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(P p) ||| (P q) = P $ \i -> let r = p i in if isErrorResult r then q i else r
...
list ::
  Parser a
  -> Parser (List a)
list p = P $ \i -> case parse p i of Result i' a -> (a :.) <$> parse (list p) i'
                                     _           -> Result i Nil
...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = bindParser (\a -> (a :.) <$> list p) p
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = bindParser (\c -> if f c then valueParser c else unexpectedCharParser c) character
...
sequenceParser (p :. ps) = bindParser (\a -> (a :.) <$> sequenceParser ps) p
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) f = bindParser $ \a -> valueParser $ f a
